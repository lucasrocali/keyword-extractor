...
function _extract(str, options) {
    ...
    var remove_links = options.remove_links;
    ...
    var remove_by_regex = options.remove_by_regex
    ...
    if (_.isEmpty(text)) {
        ...
        if (remove_links) {
            cleanedText = text.replace(/(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:/~+#-]*[\w@?^=%&/~+#-])?/g, '');
        }
        if (remove_by_regex && remove_by_regex.length > 0) {
            remove_by_regex.map(regex => cleanedText = cleanedText.replace(regex, ''))
        }
